...
const PageFiveHealthCare = ({ onButtonClick, totalCost, setTotalCost }) => {
  ...
  const [index5value, setIndex5value] = useState({
    value1: 0,
    value2: 0,
    value3: 0,
    value4: 0,
  ...
  const onClickSingleUser = (prevState) => {
    setSingleUser((prev) => {
      ...
      setIndex5value((prevState) => ({
        ...
        title1: newValue
          ? "Which of the following web components do you need(Web App)"
          : "",
        title3: "",
      }));
  ...
  const onClickMultiUser = () => {
    setMultiUser((prev) => {
      ...
      setIndex5value((prevState) => ({
        ...
        index: newValue || prevState.index ? pageIndex : 0,
        title2: newValue
          ? "Which of the following web components do you need(Backend API)"
          : "",
        title3: "",
      }));
  ...
  const onClickThirdUser = () => {
    setThirdUser((prev) => {
      ...
      setIndex5value({
        ...
        title3: newValue
          ? "Which of the following web components do you need(None)"
          : "",
  ...
  const updateCost = (single, multi, third) => {
    ...
    for (let item of costData) {
      if (item) {
        totalMin +=
          (item.value1 || 0) + (item.value3 || 0) + (item.value5 || 0);
        totalMax +=
          (item.value2 || 0) + (item.value4 || 0) + (item.value6 || 0);
      }
    }
  ...
  useEffect(() => {
    const selection = JSON.parse(
      sessionStorage.getItem("userSelection_pageSix")
    );
    if (!selection) return;

    const {
      singleUser: saveSingle,
      multiUser: saveMulti,
      thirdUser: saveThird,
    } = selection;
      ...
      <main className="pt5 black-80 body-background" style={{ margin: "auto" }}>
        ...
        <div className="center ph4 selectionDiv">
          ...
          <div
            ...
            style={{
              borderColor: singleUser ? "#18d2e8" : "#fafafa",
              height: "350px",
            }}
          ...
          <div
            className="mw5 bg-white-six br3 pa3 mv3 ba dib b--black-10 ma3 clicked page-four page_one"
            style={{
              borderColor: multiUser ? "#18d2e8" : "#fafafa",
              height: "350px",
            }}
          ...
          <div
            className="mw5 bg-white-six br3 pa3 mv3 ba dib b--black-10 ma3 clicked page-four page_one"
            style={{
              borderColor: thirdUser ? "#18d2e8" : "#fafafa",
              height: "350px",
            }}
        ...
        <div className="totals well container p-4">
          <h2 className="total-cost">
            <span className="footer-total-cost_title">
              Total Estimated Cost
            </span>
            : <span id="total-cost">{totalCost}</span>
          </h2>
          <p className="disclaimer">
            <b>Please note </b>: All cost estimates are intended to be
            indicative of development costs and timescales only and are
...
export default PageFiveHealthCare;
